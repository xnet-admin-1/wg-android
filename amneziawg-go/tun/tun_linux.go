...
func CreateUnmonitoredTUNFromFD(fd int) (Device, string, error) {
	...
	name, err := tun.Name()
	if err != nil {
		// Fall back to raw pipe/socket mode (no TUN name available)
		tun.batchSize = 1
		tun.vnetHdr = false
		tun.udpGSO = false
		return tun, "tether", nil
	}
	err = tun.initFromFlags(name)
	if err != nil {
		return nil, "", err
...
// CreateUnmonitoredTUNFromFDRaw creates a Device from a pipe/socket fd,
// skipping TUN name and flag checks. Used for tether server.
func CreateUnmonitoredTUNFromFDRaw(fd int) (Device, error) {
	err := unix.SetNonblock(fd, true)
	if err != nil {
		return nil, err
	}
	file := os.NewFile(uintptr(fd), "tether-tun")
	tun := &NativeTun{
		tunFile:                 file,
		events:                  make(chan Event, 5),
		errors:                  make(chan error, 5),
