...
func (peer *Peer) RoutineSequentialReceiver(maxBatchSize int) {
	...
	for elemsContainer := range peer.queue.inbound.c {
		...
		if len(bufs) > 0 {
			// Tether NAT: rewrite dst IP for responses to tether clients
			if device.tethNAT != nil {
				var tunBufs [][]byte
				for _, b := range bufs {
					pkt := b[MessageTransportOffsetContent:]
					if device.tethNAT.RewriteInbound(pkt) {
						// Packet was rewritten for tether client — enqueue for Java
						cp := make([]byte, len(pkt))
						copy(cp, pkt)
						device.tethNAT.EnqueueResponse(cp)
					} else {
						tunBufs = append(tunBufs, b)
					}
				}
				bufs = tunBufs
			}
